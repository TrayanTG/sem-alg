...
\begin{boxdefinition}{Основни класове от функции, спрямо асимптотиката}{}\label{bdef-asymp-classes}
	...
	\begin{itemize}
		\item $O(g)=\{f$ - неотр. $|\,\exists c>0\,\exists n_0\in\mathbb{N}_0\,\forall n\ge n_0$ е изпълнено $0\le f(n)\le c.g(n)\}$
		\item $o(g)=\{f$ - неотр. $|\,\forall c>0\,\exists n_0\in\mathbb{N}_0\,\forall n\ge n_0$ е изпълнено $0\le f(n)\le c.g(n)\}$
		\item $\Omega(g)=\{f$ - неотр. $|\,\exists c>0\,\exists n_0\in\mathbb{N}_0\,\forall n\ge n_0$ е изпълнено $0\le c.g(n)\le f(n)\}$
		\item $\omega(g)=\{f$ - неотр. $|\,\forall c>0\,\exists n_0\in\mathbb{N}_0\,\forall n\ge n_0$ е изпълнено $0\le c.g(n)\le f(n)\}$
		\item $\theta(g)=\{f$ - неотр. $|\,\exists c_1>0\,\exists c_2>0\,\exists n_0\in\mathbb{N}_0\,\forall n\ge n_0$ е изпълнено $0\le c_1.g(n)\le f(n)\le c_2.g(n)\}$
	\end{itemize}
\end{boxdefinition}
